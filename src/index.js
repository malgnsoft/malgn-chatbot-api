  ...
  async scheduled(event, env) {
    ...
    try {
      const r1 = await env.DB.prepare(
        `UPDATE TB_SESSION SET generation_status = 'failed', updated_at = CURRENT_TIMESTAMP WHERE generation_status = 'processing' AND updated_at < ${ago(10)}`
      ).run();
      if (r1.meta.changes > 0) console.log(`[Cron] ${r1.meta.changes}개 processing → failed`);

      const r2 = await env.DB.prepare(
        `UPDATE TB_SESSION SET generation_status = 'failed', updated_at = CURRENT_TIMESTAMP WHERE generation_status = 'pending' AND updated_at < ${ago(30)}`
      ).run();
