...
| 데이터베이스 | 용도 | 저장 데이터 |
|-------------|------|------------|
| **Aurora MySQL** (Hyperdrive 경유) | 메타데이터 저장 | 콘텐츠, 세션, 메시지, 퀴즈, AI 로그 |
| **Vectorize** | 벡터 저장/검색 | 콘텐츠 청크 임베딩 (1024차원) |
...
| 차원(Dimension) | 1024 (`bge-m3` 모델 기준) |
...
| cloud | `malgn-chatbot-files-cloud` |
...
# 005~010: 퀴즈 분리 / 세션 generation_status / chat_content_ids / site_id / TB_AI_LOG / lesson_id
...
```bash
# cloud 테넌트 (전용 Aurora MySQL 인스턴스)
...
### Aurora MySQL / Hyperdrive 운영 주의
- Hyperdrive는 커넥션을 풀링하므로 트랜잭션 사용 시 단일 커넥션 유지 필요
- Workers 단일 요청의 CPU 시간 한도(서브리퀘스트/소요시간) 내에 쿼리 완료 권장
- 모든 SELECT 쿼리에 `WHERE status = 1` 필수
- INTEGER 자동증가는 MySQL `AUTO_INCREMENT`, BOOLEAN 대용은 `TINYINT(1)` 사용
